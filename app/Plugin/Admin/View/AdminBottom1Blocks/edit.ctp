...
<?php echo $this->ExtendedForm->create('Bottom1Block', array('class' => 'form-horizontal', 'novalidate' => 'novalidate'));?>
    <fieldset>
        <legend><?php echo __d('admin', 'Edit Bottom1 Block'); ?></legend>
	<?php
...
<?php echo $this->ExtendedForm->end(array('label' => __d('admin', 'Save Bottom1Block'), 'class' => 'btn btn-primary', 'div' => false, 'before' => '<div class="control-group"><div class="controls">', 'after' => "\n" . $this->Html->link(__d('admin', 'Cancel'), $redirectUrl, array('class' => 'btn')) . '</div></div>'));?>
</div>